...
interface BGMComplianceSectionProps {
  userRole?: string | null
}

export function BGMComplianceSection({ userRole }: BGMComplianceSectionProps) {
  ...
  const [results, setResults] = useState<BGMComplianceResult[]>([])
  ...
  const handleProcess = async () => {
    if (!diagnosisFile || !bloodSugarFile) return
    ...
    try {
      ...
      const [diagnosis, bloodSugar] = await Promise.all([
        extractPdfText(diagnosisFile),
        extractPdfText(bloodSugarFile),
      ])

      // Send both extracted texts to the BGM compliance API
      const response = await fetch("/api/bgm-compliance-search", {
        method: "POST",
  ...
  const handleExport = async () => {
    ...
    try {
      const response = await fetch("/api/bgm-compliance-export", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          results,
          fileName: diagnosisFile?.name || "bgm-compliance",
        }),
      })
      if (!response.ok) throw new Error("Export failed")
      const blob = await response.blob()
      const fileName = `${(diagnosisFile?.name || "bgm-compliance").replace(".pdf", "")}-BGM-Compliance-${Date.now()}.docx`
  ...
  const bothFilesReady = !!diagnosisFile && !!bloodSugarFile
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
        ...
        <SCard>
          <SCardHeader
            title="Upload BGM Compliance Files"
            desc="Upload the resident's diagnosis PDF and blood sugar (BGM) log PDF to review Blood Glucose Monitor compliance."
          />
          ...
          <div style={{ padding: "16px 24px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "16px" }}>
            <div>
              <div style={{ fontSize: "12px", fontWeight: 700, color: "#374151", marginBottom: "2px" }}>
                {bothFilesReady ? "✓ Both files ready" : diagnosisFile || bloodSugarFile ? "1 of 2 files selected" : "No files selected"}
              </div>
              <div style={{ fontSize: "11px", color: "#9ca3af" }}>
                Reviews BGM compliance against diagnosis
              </div>
            ...
            <button
              ...
              style={{
                ...
                transition: "all .2s", opacity: (!bothFilesReady || isProcessing) ? 0.55 : 1,
              }}
            >
              {isProcessing ? (
                <>
                ...
                <><Droplet style={{ width: "14px", height: "14px" }} /> Analyse BGM Compliance</>
              )}
            </button>
          </div>
        </SCard>
      ...
      {results.length > 0 && (
        <SCard>
          <div style={{ padding: "20px 24px 0", display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
            <div>
              <h3 style={{ fontFamily: "'Instrument Serif',Georgia,serif", fontSize: "18px", color: "#111827", marginBottom: "4px" }}>BGM Compliance Results</h3>
              <p style={{ fontSize: "13px", color: "#9ca3af" }}>
                Found <strong style={{ color: "#1a2e6e" }}>{results.length}</strong> resident{results.length !== 1 ? "s" : ""} without BGM readings · {results.reduce((s,r)=>s+r.groups.reduce((gs,g)=>gs+g.entries.length,0),0)} diabetes diagnos{results.reduce((s,r)=>s+r.groups.reduce((gs,g)=>gs+g.entries.length,0),0)!==1?"es":"is"} flagged
              </p>
            </div>
            <button
        ...
        <SCard>
          <div style={{ padding: "48px 24px", display: "flex", flexDirection: "column", alignItems: "center", gap: "12px" }}>
            ...
            <div style={{ textAlign: "center" }}>
              <p style={{ fontFamily: "'Instrument Serif',Georgia,serif", fontSize: "16px", color: "#374151", marginBottom: "4px" }}>No BGM compliance issues found</p>
              <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: "13px", color: "#9ca3af" }}>No residents had Blood Glucose Monitor compliance issues based on the uploaded files.</p>
            </div>
          </div>
        </SCard>
      )}
