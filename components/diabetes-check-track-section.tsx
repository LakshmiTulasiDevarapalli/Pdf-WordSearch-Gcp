...
import { Download, Upload, CheckCircle2, AlertCircle, FileText, Droplet } from "lucide-react"

interface DiabetesTrackGroup {
...
interface DiabetesCheckTrackSectionProps {
  userRole?: string | null
}

export function DiabetesCheckTrackSection({ userRole }: DiabetesCheckTrackSectionProps) {
  ...
  const [bloodSugarFile, setBloodSugarFile] = useState<File | null>(null)
  const [isProcessing, setIsProcessing] = useState(false)
  const [results, setResults] = useState<DiabetesTrackResult[]>([])
  ...
  const handleDiagnosisFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0]
    if (file) {
  ...
  const handleBloodSugarFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0]
    if (file) {
  ...
  const handleProcess = async () => {
    if (!medicationFile || !diagnosisFile || !bloodSugarFile) return
    setIsProcessing(true)
    setProcessStatus("idle")
    setErrorMessage("")

    try {
      // Parse all three PDFs in-memory
      const [medication, diagnosis, bloodSugar] = await Promise.all([
        extractPdfText(medicationFile),
        extractPdfText(diagnosisFile),
        extractPdfText(bloodSugarFile),
      ])

      // Send all three extracted texts to the Diabetes Check & Track API
      const response = await fetch("/api/diabetes-check-track-search", {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          diagnosisText: diagnosis.extractedText,
          diagnosisNumPages: diagnosis.numPages,
          diagnosisFileName: diagnosisFile.name,
          bloodSugarText: bloodSugar.extractedText,
          bloodSugarNumPages: bloodSugar.numPages,
          bloodSugarFileName: bloodSugarFile.name,
        }),
      })
  ...
  const handleExport = async () => {
    ...
    try {
      const response = await fetch("/api/diabetes-check-track-export", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          results,
          fileName: diagnosisFile?.name || "diabetes-check-track",
        }),
      })
      if (!response.ok) throw new Error("Export failed")
      const blob = await response.blob()
      const fileName = `${(diagnosisFile?.name || "diabetes-check-track").replace(".pdf", "")}-Diabetes-Check-Track-${Date.now()}.docx`
  ...
  const allFilesReady = !!medicationFile && !!diagnosisFile && !!bloodSugarFile
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
        ...
        <SCard>
          <SCardHeader
            title="Upload Diabetes Check & Track Files"
            desc="Upload the resident's medication PDF, diagnosis PDF, and blood sugar log PDF to check and track diabetes monitoring."
          />
          <div style={{ padding: "16px 24px 20px", display: "flex", flexDirection: "column", gap: "16px" }}>
            ...
            <div>
              ...
              <FileDropInput
                id="diabetes-diagnosis-upload"
                file={diagnosisFile}
                onChange={handleDiagnosisFileUpload}
                disabled={isProcessing}
                label="Click to choose the diagnosis PDF file"
                hint="Resident diagnosis / condition record"
              />
            </div>
            <div>
              <label style={{ display: "block", fontSize: "11px", fontWeight: 700, color: "#374151", marginBottom: "8px", letterSpacing: "0.04em", textTransform: "uppercase" }}>
                Blood Sugar PDF File
              </label>
              <FileDropInput
                id="diabetes-blood-sugar-upload"
                file={bloodSugarFile}
                onChange={handleBloodSugarFileUpload}
                disabled={isProcessing}
                label="Click to choose the blood sugar PDF file"
                hint="Blood glucose readings log"
              />
            </div>
          </div>
        </SCard>
          ...
          <div style={{ padding: "16px 24px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "16px" }}>
            <div>
              <div style={{ fontSize: "12px", fontWeight: 700, color: "#374151", marginBottom: "2px" }}>
                {allFilesReady
                  ? "✓ All 3 files ready"
                  : [medicationFile, diagnosisFile, bloodSugarFile].filter(Boolean).length + " of 3 files selected"}
              </div>
              <div style={{ fontSize: "11px", color: "#9ca3af" }}>
                Checks diabetes tracking against medication &amp; diagnosis
              </div>
            ...
            >
              {isProcessing ? (
                ...
                <><Droplet style={{ width: "14px", height: "14px" }} /> Run Diabetes Check & Track</>
              )}
            </button>
          </div>
        </SCard>
      ...
      {results.length > 0 && (
        <SCard>
          <div style={{ padding: "20px 24px 0", display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
            <div>
              <h3 style={{ fontFamily: "'Instrument Serif',Georgia,serif", fontSize: "18px", color: "#111827", marginBottom: "4px" }}>Diabetes Check & Track Results</h3>
              <p style={{ fontSize: "13px", color: "#9ca3af" }}>
                Found <strong style={{ color: "#1a2e6e" }}>{results.length}</strong> resident{results.length !== 1 ? "s" : ""} without glucose readings · {results.reduce((s,r)=>s+r.groups.reduce((gs,g)=>gs+g.entries.length,0),0)} diabetes diagnos{results.reduce((s,r)=>s+r.groups.reduce((gs,g)=>gs+g.entries.length,0),0)!==1?"es":"is"} flagged
              </p>
            </div>
            <button
        ...
        <SCard>
          <div style={{ padding: "48px 24px", display: "flex", flexDirection: "column", alignItems: "center", gap: "12px" }}>
            ...
            <div style={{ textAlign: "center" }}>
              <p style={{ fontFamily: "'Instrument Serif',Georgia,serif", fontSize: "16px", color: "#374151", marginBottom: "4px" }}>No diabetes tracking issues found</p>
              <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: "13px", color: "#9ca3af" }}>No residents had outstanding diabetes tracking issues based on the uploaded files.</p>
            </div>
          </div>
        </SCard>
      )}
