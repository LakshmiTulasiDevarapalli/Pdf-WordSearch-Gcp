...
export async function POST(request: NextRequest) {
  try {
    ...
    children.push(
      ...
      new Paragraph({
        children: [
          new TextRun({ text: "BGM ", bold: true, font: "Times New Roman", size: 52, color: GOLD }),
          new TextRun({ text: "Compliance Review Report", bold: true, font: "Times New Roman", size: 52, color: NAVY }),
        ],
        spacing: { after: 80 },
      }),
      new Paragraph({
              ...
              new TableCell({
                children: [new Paragraph({
                  children: [
                    ...
                    new TextRun({ text: "  MISSING BGM DIAGNOSES", font: "Times New Roman", size: 20, color: "C5D1E8" }),
                  ],
                  alignment: AlignmentType.CENTER,
                })],
    ...
    for (let ri = 0; ri < results.length; ri++) {
      ...
      for (let gi = 0; gi < resident.groups.length; gi++) {
        ...
        const groupLabel = resident.groups.length > 1 ? "Issue " + (gi + 1) + ":  " : "\u25B6  Missing BGM Readings:  "
    ...
    return new NextResponse(new Uint8Array(buffer), {
      headers: {
        ...
        "Content-Disposition": "attachment; filename=\"BGM-compliance-report-" + Date.now() + ".docx\"",
      },
    })

  } catch (error) {
    console.error("[bgm-compliance-export] Error:", error)
