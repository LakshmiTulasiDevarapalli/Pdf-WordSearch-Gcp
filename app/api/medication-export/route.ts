...
export async function POST(request: NextRequest) {
  try {
    ...
    children.push(
      ...
      new Paragraph({
        children: [
          new TextRun({ text: "eMAR ", bold: true, font: "Times New Roman", size: 52, color: GOLD }),
          new TextRun({ text: "Duplicate Medication Report", bold: true, font: "Times New Roman", size: 52, color: NAVY }),
        ],
        spacing: { after: 80 },
      }),
      new Paragraph({
        children: [
          new TextRun({ text: "Type: Default PN Type for eMAR", font: "Times New Roman", size: 22, color: GRAY, italics: true }),
        ],
        spacing: { after: 40 },
      })
    )
              ...
              new TableCell({
                children: [new Paragraph({
                  children: [
                    ...
                    new TextRun({ text: "  TOTAL ENTRIES", font: "Times New Roman", size: 20, color: "C5D1E8" }),
                  ],
                  alignment: AlignmentType.CENTER,
                })],
    ...
    for (let ri = 0; ri < results.length; ri++) {
                  ...
                  children: [
                    ...
                    new Paragraph({
                      children: [
                        ...
                        new TextRun({ text: "\u2502    Admission: ", bold: true, font: "Times New Roman", size: 20, color: "C5D1E8" }),
                        new TextRun({ text: (resident.admissionDate || "N/A") + "    ", font: "Times New Roman", size: 20, color: WHITE }),
                        new TextRun({ text: "\u2502    " + resident.groups.length + " medication" + (resident.groups.length !== 1 ? "s" : ""), bold: true, font: "Times New Roman", size: 20, color: GOLD }),
                      ],
                      spacing: { after: 60 },
                    }),
                  ],
      ...
      for (let gi = 0; gi < resident.groups.length; gi++) {
        ...
        const medLabel = resident.groups.length > 1 ? "Medication " + (gi + 1) + ":  " : "\u25B6  Repeated Medication:  "
              ...
              new TableRow({
                children: [
                  new TableCell({
                    children: [new Paragraph({
                      children: [
                        new TextRun({ text: medLabel, bold: true, font: "Times New Roman", size: 20, color: GOLD }),
        ...
        rows.push(new TableRow({
          ...
          children: [
            ...
            new TableCell({
              children: [new Paragraph({ children: [new TextRun({ text: "Effective Date", bold: true, color: WHITE, font: "Times New Roman", size: 20 })], alignment: AlignmentType.CENTER })],
            ...
            new TableCell({
              children: [new Paragraph({ children: [new TextRun({ text: "Note Text", bold: true, color: WHITE, font: "Times New Roman", size: 20 })], alignment: AlignmentType.CENTER })],
    ...
    return new NextResponse(new Uint8Array(buffer), {
      headers: {
        ...
        "Content-Disposition": "attachment; filename=\"eMAR-duplicate-report-" + Date.now() + ".docx\"",
      },
    })

  } catch (error) {
    console.error("[medication-export] Error:", error)
