...
export async function POST(request: NextRequest) {
  try {
    ...
    children.push(
      ...
      new Paragraph({
        children: [
          new TextRun({ text: "Sugar ", bold: true, font: "Times New Roman", size: 52, color: GOLD }),
          new TextRun({ text: "Sense Report", bold: true, font: "Times New Roman", size: 52, color: NAVY }),
        ],
        spacing: { after: 80 },
      }),
      new Paragraph({
        children: [
          new TextRun({ text: "Top blood sugar readings for residents on antidiabetic medication (sliding-scale and subcutaneous insulin residents excluded)", font: "Times New Roman", size: 22, color: GRAY, italics: true }),
        ],
        spacing: { after: 40 },
      })
    )
              ...
              new TableCell({
                children: [new Paragraph({
                  children: [
                    ...
                    new TextRun({ text: "  HIGH READINGS FLAGGED", font: "Times New Roman", size: 20, color: "C5D1E8" }),
                  ],
                  alignment: AlignmentType.CENTER,
                })],
    ...
    for (let ri = 0; ri < results.length; ri++) {
                  ...
                  children: [
                    ...
                    new Paragraph({
                      children: [
                        ...
                        new TextRun({ text: "\u2502    " + resident.groups.reduce((s,g)=>s+g.entries.length,0) + " reading" + (resident.groups.reduce((s,g)=>s+g.entries.length,0) !== 1 ? "s" : ""), bold: true, font: "Times New Roman", size: 20, color: GOLD }),
                      ],
                      spacing: { after: 60 },
                    }),
                  ],
      ...
      for (let gi = 0; gi < resident.groups.length; gi++) {
        ...
        const groupLabel = resident.groups.length > 1 ? "Group " + (gi + 1) + ":  " : "\u25B6  Top Blood Sugar Readings:  "
        ...
        rows.push(new TableRow({
          ...
          children: [
            ...
            new TableCell({
              children: [new Paragraph({ children: [new TextRun({ text: "Reading Date/Time", bold: true, color: WHITE, font: "Times New Roman", size: 20 })], alignment: AlignmentType.CENTER })],
            ...
            new TableCell({
              children: [new Paragraph({ children: [new TextRun({ text: "Blood Sugar", bold: true, color: WHITE, font: "Times New Roman", size: 20 })], alignment: AlignmentType.CENTER })],
        ...
        group.entries.forEach((entry, idx) => {
          ...
          rows.push(new TableRow({
            children: [
              ...
              new TableCell({
                children: [new Paragraph({ children: [new TextRun({ text: entry.noteText, font: "Times New Roman", size: 20, bold: true, color: "B91C1C" })] })],
                width: { size: 6200, type: WidthType.DXA },
    ...
    return new NextResponse(new Uint8Array(buffer), {
      headers: {
        ...
        "Content-Disposition": "attachment; filename=\"Sugar-Sense-report-" + Date.now() + ".docx\"",
      },
    })

  } catch (error) {
    console.error("[sugar-sense-export] Error:", error)
